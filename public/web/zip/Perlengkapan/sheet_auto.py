...
def update_google_sheets(data):
    ...
    # ✅ Deteksi otomatis tab Google Sheets
    tabs = driver.window_handles
    found = False
    for handle in tabs:
        driver.switch_to.window(handle)
        if "docs.google.com/spreadsheets" in driver.current_url:
            print(f"📄 Tab Google Sheets terdeteksi: {driver.current_url}")
            found = True
            break

    if not found:
        print("⚠️ Tidak ada tab Google Sheets terbuka. Harap buka dulu sebelum menjalankan agent.")
        return

    # Setelah tab aktif, lanjut otomatis
