...
export default function App() {
  ...
  const handleSaveExcelPath = async () => {
    if (!excelPath) return;
    setMessage({ type: '', text: '' });
    try {
      const resp = await fetch(`${apiUrl.trim()}/excel_path`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (resp.ok && data.status === 'success') {
        setMessage({ type: 'success', text: `Excel path berhasil disimpan!` });
      } else {
        setMessage({ type: 'error', text: data.message || 'Gagal menyimpan Excel path' });
      }
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal menghubungi API' });
    }
  };
            ...
            <div className="mb-6">
              ...
              <div className="flex space-x-2">
                <input
                  ...
                  placeholder="C:\\raja_iblis\\static\\DAFTAR PROJECT E-KATALOG.xlsx"
                />
                <button
                  onClick={handleSaveExcelPath}
                  className="bg-orange-500 hover:bg-orange-600 text-white font-medium px-4 rounded-lg"
                >
                  Simpan
                </button>
              </div>
